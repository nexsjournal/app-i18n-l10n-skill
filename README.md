...
支持两种语言消费模式：**仅系统语言**（`system_only`）与 **应用内设置页切换**（`in_app_switch`，含 `AppLanguage`、`AppleLanguages`、三层运行时消费链）。
...
# 审计 SwiftUI 运行时本地化（持久化 key、AppleLanguages、动态文案等）
python3 scripts/audit_runtime_l10n.py --source-dir path/to/MyApp
python3 scripts/audit_runtime_l10n.py --source-dir path/to/MyApp --require-in-app-switch
